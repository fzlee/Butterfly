...
class BaseUserService(BaseService):
    ...
    def set_login_credential_to_cookie(cls, response, user):
        ...
        response.set_cookie("b.token", user.token.key, max_age=max_age, httponly=False)
        user = {
            "uid": user.uid,
        ...
        response.set_cookie("b.user", data, max_age=max_age)
        return response
    ...
    def generate_login_credential(cls, user):
        return {
            ...
            "b.token": user.token.key
        }

    @classmethod
    def clear_login_credential(cls, request, response):
        response.delete_cookie("b.token")
        response.delete_cookie("b.user")
